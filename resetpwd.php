...
session_start();
$ErrMsg = '';
if(!isset($_SESSION['forgetID']))echo "<script> {window.alert('請重新寄送郵件');location.href='forget.php'} </script>";
$forgetID=$_SESSION['forgetID'];
...
if(isset($Submit) && isset($vCode)){
	...
	{
		$PWD=sha1($PWD);
		$sqlcmd="UPDATE userLP SET password='$PWD' WHERE loginid='$forgetID'";
		$result = updatedb($sqlcmd, $db_conn);
		unset($_SESSION['forgetID']);
		echo "<script> {window.alert('修改成功');location.href='login.php'} </script>";
	}
$vCode ='';
