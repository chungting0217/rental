...
<style>
	...
	<?php
		...
		if(isset($confirm))
		{
			...
			$sqlcmd="SELECT * FROM spaces WHERE sname = '$sname' AND mid='$mid'";
    		$rs = querydb($sqlcmd, $db_conn);
    		if(count($rs)<=0)$ErrMsg.= '沒有此場館\n';
			...
			if(empty($ErrMsg))
			{
				...
				// 	echo $date;
				// }
				 $sqlcmd='INSERT INTO specialspaces (mid,sname,sphone,semail,caddress,address,date,start,end) VALUES ('
				 	."?,?,?,?,?,?,?,?,?)";
					...
					$_SESSION['day']=$date;
				  	...
				  	$rs->execute(array($mid,$sname,$telNo,$semail,$city,$address,$date,$start,$end));
				  	
				 	
				  	$uid = $db_conn->lastInsertId();
				  	
				  		$sqlcmd="UPDATE specialspaces SET sid='$uid' WHERE date='$date' AND sname='$sname'";
				  ...
				  for($j=0;$j<count($sport);$j++)
					{
						...
						$sqlcmd="UPDATE specialspacessport SET sportid='$sportid' WHERE sid='$uid' AND sport='$nowsport'";
        		 		$result = updatedb($sqlcmd, $db_conn);
					}
